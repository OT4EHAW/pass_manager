function displayPasswords(passwords) {
                    ...
                    <div class="accordion-item" id="${passwords[i].id}">
                      ...
                      <div id="collapse-${passwords[i].id}" class="accordion-collapse collapse" aria-labelledby="heading-${passwords[i].id}" data-bs-parent="#passwords">
                        <div class="accordion-body">
                            ...
                            <br>
                            Логин: <span class="password-username">${passwords[i].username}</span>
                            <br>
                            <button class="btn btn-primary copy-button">Копировать пароль</button>
                            <span style="display: none" class="encrypted-password">
                                ${passwords[i].encrypted_password}
                            </span>
                            <br>
                            Ссылка: <a href="${passwords[i].url}" target="_blank" rel="noopener noreferrer">
    ...
    if (content == "")
        content = "<h2 class='mx-auto w-75'>У Вас нет сохраненных паролей!</h2>";
    ...
    copyButtons.forEach(button => {
        button.onclick = () => {
            ...
            modalClose("Введите мастер пароль чтобы скопировать", decryptAndCopyPassword, encrypted_password);
        }
    })
    editPasswordEvent();
    deletePasswordEvent();
